...
def _message_area_widgets(message_area_frm):
    ...
    """
    message_ctrl_label = ttk.Label(
        message_area_frm, borderwidth=0, anchor=tk.CENTER, style="Check.TLabel")
...
def _buttons_frame_widgets(buttons_frm, messages_bundle):
    ...
    * "cancel", to close the student tab with no further action.
    ...
    # The clear button.
    clb.add_button("clear_btn", ttk.Button(
        buttons_frm, text=messages_bundle["clear_button"], command=clb.clear_action))
    clb.buttons["clear_btn"].grid(row=0, column=2, padx=10, pady=10)

    # The cancel button.
    clb.add_button("cancel_btn", ttk.Button(
        buttons_frm, text=messages_bundle["cancel_button"], command=clb.cancel_action))
...
def _add_labels(student_frm, messages_bundle):
    ...
    ttk.Label(student_frm, text=messages_bundle["stud_number"] + " *").grid(
        row=0, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["first_name"] + " *").grid(
        row=1, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["last_name"] + " *").grid(
        row=2, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["gender"] +
              " *").grid(row=3, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["email_address"] + " *").grid(
        row=4, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["alternate_email_address"]).grid(
        row=5, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["first_association"]).grid(
        row=6, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["second_association"]).grid(
        row=7, padx=10, pady=10, sticky='W')
    ttk.Label(student_frm, text=messages_bundle["third_association"]).grid(
        row=8, padx=10, pady=10, sticky='W')
...
def _add_control_labels(student_frm, lang):
    ...
    clb.control_labels["stud_number_ctrl"].grid(
    ...
    clb.add_control_label("first_name_ctrl", ttk.Label(student_frm, width=control_labels_width[lang],
                                                       borderwidth=0, style="Check.TLabel"))
    clb.control_labels["first_name_ctrl"].grid(
...
def _add_entries(student_frm):
    ...
    stud_number = tk.StringVar(value="")
    stud_number.trace("w",
    ...
    email_addresses = [tk.StringVar(value="") for _ in range(2)]
    ...
    email_addresses_ent = [
        ttk.Entry(student_frm, textvariable=email_addresses[i]) for i in range(2)]
    clb.add_entry("email_addresses", [
                  (email_addresses_ent[i], email_addresses[i]) for i in range(2)])
    init_row = 4
    for i in range(2):
...
def _add_combo_boxes(student_frm, cursor):
    ...
    associations = [""] + \
        (list(map(lambda x: x[0], mstud.get_associations(cursor))))
    stud_roles = mstud.get_roles(cursor)

    # We create three combo boxes for the associations and three for the student roles.
    asso_names_combo = [ttk.Combobox(
        student_frm, values=associations, state="readonly") for _ in range(3)]
    stud_roles_combo = [ttk.Combobox(
        student_frm, values=stud_roles, state="readonly") for _ in range(3)]
    ...
    for i in range(3):
        ...
        stud_roles_combo[i].grid(
            row=row_index+i, column=2, padx=10, sticky='W')
