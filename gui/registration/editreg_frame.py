...
Definition of the tab where the user can edit a new registration to a Pistus edition.
"""
...
def add_widgets(edit_reg_tab, messages_bundle, cursor, conn, lang):
    ...
    scroll_registration_frm = registration_widgets(
        registration_frm, messages_bundle)
    ...
    clb.init(messages_bundle, check_image, edit_reg_tab,
             scroll_registration_frm, cursor, conn)
    clb.reset()
...
def student_widgets(student_frm, messages_bundle, lang):
    ...
    first_name_entry = ttk.Entry(
        student_frm, state="disabled", textvariable=first_name)
    ...
    last_name_entry = ttk.Entry(
        student_frm, state="disabled", textvariable=last_name)
...
def registration_widgets(registration_frm, messages_bundle):
    ...
    scrollbar = ttk.Scrollbar(
        registration_frm, orient="vertical", command=canvas.yview)
    ...
    canvas_frm = canvas.create_window(
        (0, 0), window=scroll_registration_frm, anchor="center")
