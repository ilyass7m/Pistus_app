...
Definition of the tab where the user can add a new registration to a Pistus edition.
"""
...
def _student_widgets(data_fields_frm, messages_bundle, lang):
    ...
    # The labels.
    ttk.Label(data_fields_frm, text=messages_bundle["stud_number"] + " *").grid(
        row=0, column=0, padx=10, pady=10, sticky='w')
    ttk.Label(data_fields_frm, text=messages_bundle["first_name"]).grid(
        row=1, column=0, padx=10, pady=10, sticky='w')
    ttk.Label(data_fields_frm, text=messages_bundle["last_name"]).grid(
    ...
    first_name_entry = ttk.Entry(
        data_fields_frm, state="disabled", textvariable=first_name)
    ...
    last_name_entry = ttk.Entry(
        data_fields_frm, state="disabled", textvariable=last_name)
    ...
    stud_number_control_label = ttk.Label(
        data_fields_frm, borderwidth=0, width=control_labels_width[lang], style="Check.TLabel")
    ...
    ttk.Label(data_fields_frm, borderwidth=0, width=control_labels_width[lang], style="Check.TLabel").grid(
...
def _edition_widgets(data_fields_frm, messages_bundle, lang):
    """Creates the widgets relative to the Pistus edition (year, registration fee, registration and payment date).
    ...
    # The labels.
    ttk.Label(data_fields_frm, text=messages_bundle["edition_year"] + " *").grid(
        row=3, column=0, padx=10, pady=10, sticky='w')
    ttk.Label(data_fields_frm, text=messages_bundle["registration_fee"] + " *").grid(
        row=4, column=0, padx=10, pady=10, sticky='w')
    ttk.Label(data_fields_frm, text=messages_bundle["registration_date"] + " *").grid(
        row=5, column=0, padx=10, pady=10, sticky='w')
    ttk.Label(data_fields_frm, text=messages_bundle["payment_date"]).grid(
    ...
    year_entry.bind('<FocusOut>', lambda event: clb.find_pistus_edition(event))
    clb.add_year_entry((year_entry, year))

    # The registration fee text field.
    registration_fee = tk.StringVar(value="")
    registration_fee.trace("w",
                           lambda name, index, mode: clb.registration_fee_updated())
    registration_fee_entry = ttk.Entry(
        data_fields_frm, textvariable=registration_fee)
    ...
    registration_date = tk.StringVar(value="")
    registration_date.trace("w",
                            lambda name, index, mode: clb.registration_date_updated())
    registration_date_entry = ttk.Entry(
        data_fields_frm, textvariable=registration_date)
    registration_date_entry.grid(row=5, column=1, sticky='w')
    clb.add_registration_date_entry(
        (registration_date_entry, registration_date))

    # The payment date text field.
    payment_date = tk.StringVar(value="")
    ...
    registration_date_control_label = ttk.Label(
        data_fields_frm, borderwidth=0, width=control_labels_width[lang], style="Check.TLabel")
    ...
    payment_date_control_label = ttk.Label(
        data_fields_frm, borderwidth=0, width=control_labels_width[lang], style="Check.TLabel")
